...
for n = 1 : N
    ...
    if user_input
        ...
        [data.seeds{n}, data.seed_info{n}, data.ground_truth{n}, params] = TDGAutoInput(data.ground_truth{n}, params, n);
    end
    if debug.enable
        figure;
        subplot(1,2,1);
...
for n = 1 : N
    ...
    seeds = data.seeds{n};
    seed_info = data.seed_info{n};
    M = size(seeds, 1);
    iou = zeros(M,1);
    iou_valid = zeros(M,1);
    for m = 1:M
        gt_label = seed_info(m).label;
    ...
    results.jaccard_valid_seeds(n) = mean(iou(seed_info.inside_cell));
    results.dice_valid_seeds(n) = 2*results.jaccard_valid_seeds(n) / (1 + results.jaccard_valid_seeds(n));
    
end
disp('jaccard');
disp(results.jaccard_all);
disp('dice');
disp(results.dice_all);