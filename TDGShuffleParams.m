...
random_frac     = linspace(1 - 1/shuffle_rate,1+1/shuffle_rate,N);
method_dict     = ['kde','gmm','voronoi'];



switch params.cell_dataset
	case 'fluo-c2dl-msc'
		params.th 			                = 0.012+0.01.*abs(normrnd(0,log10(shuffle_rate),1,N));
		...
		params.pp.remove_bg_lighting.sigma  = 100*random_frac(rand_perm(length(random_frac)));
		params.pp.median_filter.enable      = random_logic(randperm(length(random_logic)));
		params.pp.median_filter.size        = [3 3]*random_frac(rand_perm(length(random_frac)));
		params.pp.gaussian_filter.enable    = random_logic(randperm(length(random_logic),N));
        params.pp.gaussian_filter.sigma     = 3*random_frac(rand_perm(length(random_frac)));
